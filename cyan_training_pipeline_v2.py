...
bars = ax.barh(has_missing['column'], has_missing['missing_pct'],
               color=bar_colors, edgecolor='#000000', linewidth=1.2, alpha=0.9)
...
for i, col in enumerate(key_cats):
    ...
    axes[i].bar(vc.index.astype(str), vc.values,
                color='#848484', edgecolor='#000000', linewidth=1.2, alpha=0.85)
...
axes[0].bar(hourly.index, hourly.values, color='#848484', edgecolor='#000000', linewidth=1.2)
axes[0].set_title('Incidents by Hour of Day', fontweight='bold')
axes[0].set_xlabel('Hour'); axes[0].set_ylabel('Count')
axes[0].axvspan(7, 10, alpha=0.15, color='#fb923c', label='Morning rush')
axes[0].axvspan(17, 21, alpha=0.15, color='#facc15', label='Evening rush')
axes[0].legend()

days = ['Mon','Tue','Wed','Thu','Fri','Sat','Sun']
axes[1].bar([days[int(d)] for d in daily.index], daily.values,
            color='#000000', edgecolor='#000000', linewidth=1.2)
...
# Initialize location_cluster placeholder (will be fit and assigned after train-test split)
...
# == FINAL feature list (removed post-event leakage and dominant bias columns) ==
CANDIDATE_FEATURES = [
    # == Spatial (0% null) ==========================
...
# == Build geolookup database (independent of train/test) ==========================
...
# Split raw data before any encoding, standard scaling, or KMeans fits to prevent leakage
...
# 1. Fit KMeans clustering ONLY on training coordinates
print("\nFitting KMeans geo-clustering on training coordinates...")
valid_geo_train = X_train[(X_train['latitude'] > 0) & (X_train['longitude'] > 0)][['latitude', 'longitude']]
N_CLUSTERS = min(20, max(5, len(valid_geo_train) // 200))
kmeans = KMeans(n_clusters=N_CLUSTERS, random_state=42, n_init=10)
kmeans.fit(valid_geo_train)

# Assign clusters
X_train['location_cluster'] = kmeans.predict(X_train[['latitude', 'longitude']])
X_test['location_cluster'] = kmeans.predict(X_test[['latitude', 'longitude']])
joblib.dump(kmeans, OUTPUT_DIR / 'kmeans_model.pkl')
print(f"[OK] {N_CLUSTERS} geo-clusters created on training set.")

# Cluster visualisation (using the plasma color map to represent multiple clusters beautifully)
plt.figure(figsize=(10, 8))
scatter = plt.scatter(valid_geo_train['longitude'], valid_geo_train['latitude'],
                      c=X_train.loc[valid_geo_train.index, 'location_cluster'],
...
# 2. Encode categoricals ONLY using training data distribution
...
for col in cat_cols:
    ...
    # Handle unseen categories in the test set by mapping them to the default 'UNKNOWN'
    classes = set(le.classes_)
    default_class = 'UNKNOWN' if 'UNKNOWN' in classes else list(classes)[0]
    
    X_train[col] = X_train[col].apply(lambda x: x if x in classes else default_class)
    X_test[col] = X_test[col].apply(lambda x: x if x in classes else default_class)
...
# 3. Fill remaining numeric nulls with column median from training set
...
df_corr_check[TARGET] = y_train
corr_with_target = (df_corr_check.corr()[TARGET]
                    .drop(TARGET)
                    .abs()
                    .sort_values(ascending=False))

plt.figure(figsize=(8, max(5, len(corr_with_target) * 0.35)))
corr_with_target.plot(kind='barh', color='#000000', edgecolor='#000000', linewidth=1.2, alpha=0.85)
...
for c, n in y_train.value_counts().sort_index().items():
    print(f"  {c}  {SEVERITY_LABELS[c]:10}: {n:5,}  ({n/len(y_train)*100:.1f}%)")

# 4. Standard Scaler (Fit only on train, transform both)
...
colors_fi = ['#f87171' if imp > fi_df['importance'].quantile(0.75)
             else '#848484' for imp in fi_df['importance']]
ax.barh(fi_df['feature'], fi_df['importance'],
        color=colors_fi, edgecolor='#000000', linewidth=1.2)
...
ax1.plot(mlp.loss_curve_, label='Training Loss', color='#f87171', linewidth=2)
ax1.set_xlabel('Epoch')
ax1.set_ylabel('Loss', color='#f87171')
...
if hasattr(mlp, 'validation_scores_') and mlp.validation_scores_:
    ...
    ax2.set_ylabel('Val Accuracy', color='#4ade80')
...
if TABNET_AVAILABLE:
    ...
    fi_tn.plot(kind='barh', color='#000000', edgecolor='#000000', linewidth=1.2)
...
bar_colors_m = ['#848484', '#fb923c', '#facc15', '#000000', '#f87171']
metrics = ['Accuracy', 'F1-Macro', 'F1-Weighted']

for i, metric in enumerate(metrics):
    vals = res_df[metric]
    bars = axes[i].bar(vals.index, vals.values,
                       color=bar_colors_m[:len(vals)], edgecolor='#000000', linewidth=1.2, alpha=0.85)
